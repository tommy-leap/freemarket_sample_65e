class Step1 < ApplicationRecord
  ...
  validates :email, presence: true, uniqueness: true, format: { with: /\A([\A@\s]+)@((?:[-a-z0-9]+\.)+[a-z]{2,})\Z/i}
  validates :encrypted_password, presence: true, uniqueness: true, format: { with: /\A(?=.*?[a-z])(?=.*?\d)[a-z\d]{8,100}\z/i}
